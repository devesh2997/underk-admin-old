...
class ReturnOrderItem extends Component {
	constructor (props) {
		...
		let returnedProductSkus = []
		const orderProductSkus = Object.keys(props.order.products)
		orderProductSkus.forEach(opSku => {
			let op = props.order.products[opSku]
			if (
				op.delivery.status === types.DELIVERY_STATUS_OUT_FOR_PICKUP ||
				op.delivery.status ===
					types.DELIVERY_STATUS_RETURN_MANIFESTED ||
				op.delivery.status ===
					types.DELIVERY_STATUS_RETURN_IN_TRANSIT ||
				op.delivery.status === types.DELIVERY_STATUS_RETURNED ||
				op.delivery.status ===
					types.DELIVERY_STATUS_CANCELLED_RETURN_IN_TRANSIT ||
				op.delivery.status === types.DELIVERY_STATUS_CANCELLED_RETURNED
			) {
				returnedProductSkus.push(opSku)
			}
		})
		this.state = {
			collapsed: true,
