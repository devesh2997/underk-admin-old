...
const isWidgetTypeWithChildren = type => {
	return (
		type === WIDGET_TYPES.COOLSLIDER ||
		type === WIDGET_TYPES.CAROUSEL ||
		type === WIDGET_TYPES.GRID
	)
}
...
class AddWidget extends React.Component {
	constructor (props) {
		...
		this.state = {
			type: '',
			priority: props.location.state
				? props.location.state.priority.toString()
				: '0',
			childAspectRatio: '1',
			itemToScreenRatio: '',
			backgroundColor: '',
	...
	onSubmit = async event => {
		...
		if (isWidgetTypeWithChildren(type)) {
			let children = []
			for (let i = 0; i < this.state.children.length; i++) {
				...
				if (
					this.state.children[i].destinationType &&
					this.state.children[i].destination
				) {
					child.helm = {
				...
				if (this.state.children[i].backgroundColor) {
					child.backgroundColor = this.state.children[
						i
					].backgroundColor
				}
				child.src = await this.uploadTaskPromise(
					this.state.children[i].srcFile,
					this.props.firebase
				)
				if (this.state.children[i].placeholder) {
					child.placeholder = await this.uploadTaskPromise(
						this.state.children[i].placeholderFile,
						this.props.firebase
					)
				}
				children.push(child)
				...
				<CardBody>
					<Form onSubmit={this.onSubmit}>
						...
						{loadingCategories || loadingCollections ? (
							...
							<div
								className='flex-card_container'
								style={{ display: 'block' }}
							>
								<h5 style={{ margin: '1rem 1.25rem 0.5rem' }}>
									Children
								</h5>
								...
								<ChildCard
									...
									collections={collections}
									onAction={childData =>
										this.handleChildAddition(childData)
									}
								/>
							</div>
						)}
				...
				return (
					...
					>
						...
						{this.props.categories.map(category => (
							<option key={category.cid} value={category.slug}>
								{category.slug}
							</option>
						))}
					</Input>
