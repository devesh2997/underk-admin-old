...
const AddSize = (props) => {
	return (
		<div>
			<div>
				<label>Size</label>
				<select name="size" value={props.currentSize} onChange={props.onSizeSelectionChange} type="text">
			...
			<button disabled={props.isSizeSelectionInvalid} onClick={props.onSizeSubmit} className="admn_pnl-button">
				Submit size
			</button>
		</div>
	);
...
class ClothingForm extends Component {
	...
	componentDidMount() {
		...
		this.onSubmit = event => {
			...
			addProduct({ ...product }, this.props.firebase).then((value) => {
	...
	render() {
		const { color, style, sizes, design, colors, sizeOptions, designs, styles, } = this.state;
		...
		const isInvalidForAddingSize = color === "" || style === "" || design === "" || product.type === "" || product.title === "" || product.category === "" || product.supplier === "" || product.slug === "" || product.listPrice === "" || product.sku === "" || product.gender === "";

		const isInvalid = color === "" || style === "" || design === "" || Object.keys(sizes).length === 0 || product.type === "" || product.title === "" || product.category === "" || product.supplier === "" || product.slug === "" || product.listPrice === "" || product.sku === "" || product.gender === "";
			...
			<div>
				{(loadingColors || loadingSizes || loadingStyles || loadingDesigns)
					...
					: <form onSubmit={this.onSubmit} className="admn_pnl-form">
						<div>
							<label>Color</label>
							<select name="color" value={color} onChange={this.onChange} type="text">
								<option value="" defaultValue>
									Select color
								</option>
								{
						...
						<div>
							<label>Style</label>
							<select name="style" value={style} onChange={this.onChange} type="text">
								<option value="" defaultValue>
									Select style
							</option>
								{
						...
						<div>
							<label>Design</label>
							<select name="design" value={design} onChange={this.onChange} type="text">
								<option value="" defaultValue>
									Select design
							</option>
								{
...

						{
							addingSize ?
								...
								/> :
								<button disabled={isInvalidForAddingSize} onClick={this.addSizeButtonClicked} className="admn_pnl-button">
									Add Size
							</button>
						}

						{!addingSize && <button disabled={isInvalid} type="submit" className="admn_pnl-button">
							Submit
						</button>}
					</form>
				}
